function [ftRank,ftScore,type] = ftSel_SVMRFECBR(ft,label,param)
...
SVMtype = 0; % SVM type, see libsvm. C-SVM: 0; nu-SVM: 1
...
while ~isempty(ftIn)
	...
	nFtIn = length(ftIn);
	[supVec,alpha_signed,type] = trainSVM(ft(:,ftIn),label,kerOpt);
	
	nSv = size(supVec,1);
	svInProd = supVec*supVec';
	svSqr = sum(supVec.^2,2);
	kerMatAll0 = repmat(svSqr,1,nSv) + repmat(svSqr',nSv,1) - 2*svInProd;
	w2_allIn = alpha_signed' * exp(-kerOpt.g * kerMatAll0) * alpha_signed;
	...
	if useApproxAlpha
		...
		for iFtIn = 1:nFtIn
			...
			w2_in(iFtIn) = alpha_signed'*kerMatRemoveP*alpha_signed;
		...
		for iFtIn = 1:nFtIn
			...
			ftInWoP = ftIn;
			ftInWoP(iFtIn) = [];
			[supVecWoP,alpha_signedWoP,svIdxsWoP,type] = trainSVM(ft(:,ftInWoP),label,kerOpt);
			...
			w2_in(iFtIn) = alpha_signedWoP' * exp(-kerOpt.g * kerMatAll0WoP) *...
				alpha_signedWoP;
...
function [supVec,alpha_signed,svIdxs,type] = trainSVM(X,Y,kerOpt)
% use libsvm to find the support vectors and alphas
if kerOpt.S==0
    type = sprintf('-s %d -t %d -c %f -g %f -q',kerOpt.S,kerOpt.type,kerOpt.C,kerOpt.g);%-g %f %,kerOpt.g
elseif kerOpt.S==1
    type = sprintf('-s %d -t %d -c %f -n %f -g %f -q',kerOpt.S,kerOpt.type,kerOpt.C,kerOpt.N,kerOpt.g);%-g %f %,kerOpt.g
end
